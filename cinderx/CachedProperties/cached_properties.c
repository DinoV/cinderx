...
static int cached_classproperty_traverse(
...
cached_classproperty_new(PyTypeObject* type, PyObject* args, PyObject* kwds) {
  ...
  if (PyTuple_GET_SIZE(args) != 1) {
    PyErr_SetString(
        PyExc_TypeError, "cached_classproperty: 1 argument expected");
    return NULL;
  }
...
cached_classproperty_get(PyObject* self, PyObject* obj, PyObject* cls) {
  PyCachedClassPropertyDescrObject* cp =
      (PyCachedClassPropertyDescrObject*)self;
  PyObject* res;
...
static void cached_classproperty_dealloc(PyCachedClassPropertyDescrObject* cp) {
...
static PyObject* cached_classproperty_get___doc__(
    PyCachedClassPropertyDescrObject* cp,
    void* closure) {
...
static PyObject* cached_classproperty_get_name(
    PyCachedClassPropertyDescrObject* cp,
    void* closure) {
...
static int cached_property_traverse(
...
cached_property_init(PyObject* self, PyObject* args, PyObject* kwds) {
  ...
  if (PyTuple_GET_SIZE(args) != 1 && PyTuple_GET_SIZE(args) != 2) {
    PyErr_SetString(
        PyExc_TypeError, "cached_property: 1 or 2 arguments expected");
    return -1;
  }
  ...
  if (PyTuple_GET_SIZE(args) == 2) {
    ...
    if (Py_TYPE(name_or_descr) != &PyMemberDescr_Type) {
      PyErr_SetString(
          PyExc_TypeError,
          "cached_property: member descriptor expected for 2nd argument");
      return -1;
    }

    descr = (PyMemberDescrObject*)name_or_descr;
    if (descr->d_member->type != T_OBJECT_EX || descr->d_member->flags) {
      PyErr_SetString(
          PyExc_TypeError, "cached_property: incompatible descriptor");
      return -1;
    }
...
static int cached_property_set(PyObject* self, PyObject* obj, PyObject* value) {
  PyCachedPropertyDescrObject* cp = (PyCachedPropertyDescrObject*)self;
  PyObject** dictptr;

  if (Py_TYPE(cp->name_or_descr) == &PyMemberDescr_Type) {
    return Py_TYPE(cp->name_or_descr)
        ->tp_descr_set(cp->name_or_descr, obj, value);
  }
...
static PyObject* cached_property_get_name(
    PyCachedPropertyDescrObject* cp,
    void* closure) {
  PyObject* res;
...
static PyObject* cached_property_get_slot(
    PyCachedPropertyDescrObject* cp,
    void* closure) {
  if (Py_TYPE(cp->name_or_descr) == &PyMemberDescr_Type) {
...
static PyObject* cached_property_clear(
    PyCachedPropertyDescrObject* self,
    PyObject* obj) {
  PyCachedPropertyDescrObject* cp = (PyCachedPropertyDescrObject*)self;
  PyObject** dictptr;

  if (Py_TYPE(cp->name_or_descr) == &PyMemberDescr_Type) {
    if (Py_TYPE(cp->name_or_descr)->tp_descr_set(cp->name_or_descr, obj, NULL) <
        0) {
      if (PyErr_ExceptionMatches(PyExc_AttributeError)) {
        PyErr_Clear();
        Py_RETURN_NONE;
...
static PyObject* cached_property_has_value(
    PyCachedPropertyDescrObject* self,
    PyObject* obj) {
  PyCachedPropertyDescrObject* cp = (PyCachedPropertyDescrObject*)self;
  PyObject** dictptr;
  ...
  if (Py_TYPE(cp->name_or_descr) == &PyMemberDescr_Type) {
    PyObject* value =
        Py_TYPE(cp->name_or_descr)
            ->tp_descr_get(cp->name_or_descr, obj, (PyObject*)Py_TYPE(obj));
...
static PyObject* cached_property___set_name__(
    PyObject* self,
    PyObject* const* args,
    Py_ssize_t nargs) {
  if (nargs != 2) {
    PyErr_SetString(
        PyExc_TypeError, "cached_property.__set_name__: 2 arguments expected");
    return NULL;
  }
  PyCachedPropertyDescrObject* cp = (PyCachedPropertyDescrObject*)self;
  ...
  if (cp->name_or_descr != Py_None) {
    if (PyUnicode_CheckExact(cp->name_or_descr)) {
      ...
      // This cannot normally happen in managed code, unless someone manually
      // calls `__set_name__` after a slot-backed property was defined (see
      // test_cached_property_set_name_on_slot_backed_property)
      PyErr_Format(
          PyExc_RuntimeError,
...
static PyMethodDef cached_property_methods[] = {
    ...
    {"__set_name__",
     (PyCFunction)cached_property___set_name__,
     METH_FASTCALL,
     NULL},
    {NULL, NULL}};
...
static int async_cached_property_traverse(
...
static int async_cached_property_init_impl(
    PyAsyncCachedPropertyDescrObject* self,
    PyObject* func,
...
/*[clinic end generated code: output=d8f17f423e7ad7f2 input=5bfa6f9a771d138d]*/
{
  if (name_or_descr != NULL) {
    ...
    if (descr->d_member->type != T_OBJECT_EX || descr->d_member->flags) {
      PyErr_SetString(
          PyExc_TypeError, "async_cached_property: incompatible descriptor");
      return -1;
    }
...
async_cached_property_get(PyObject* self, PyObject* obj, PyObject* cls) {
  PyObject* res;
  PyAsyncCachedPropertyDescrObject* cp =
      (PyAsyncCachedPropertyDescrObject*)self;
  ...
  if (Py_TYPE(cp->name_or_descr) == &PyMemberDescr_Type) {
    ...
    res =
        PyObject_CallFunctionObjArgs(_AsyncLazyValue_Type, cp->func, obj, NULL);
    if (res == NULL) {
      return NULL;
    }
...
static void async_cached_property_dealloc(
    PyAsyncCachedPropertyDescrObject* cp) {
  PyObject_GC_UnTrack(cp);
  Py_XDECREF(cp->func);
  Py_XDECREF(cp->name_or_descr);
...
static PyObject* async_cached_property_get_name(
    PyAsyncCachedPropertyDescrObject* cp,
    void* closure) {
  PyObject* res;
...
static PyObject* async_cached_property_get_slot(
    PyAsyncCachedPropertyDescrObject* cp,
    void* closure) {
  if (Py_TYPE(cp->name_or_descr) == &PyMemberDescr_Type) {
    PyObject* res = cp->name_or_descr;
...
static PyMemberDef async_cached_property_members[] = {
    {"func",
     T_OBJECT,
     offsetof(PyAsyncCachedPropertyDescrObject, func),
     READONLY},
    {"fget",
     T_OBJECT,
     offsetof(PyAsyncCachedPropertyDescrObject, func),
...
PyDoc_STRVAR(
    async_cached_property_init__doc__,
    "async_cached_property(func, name_or_descr=None)\n"
    "--\n"
    "\n"
    "init a async_cached_property.\n"
    "\n"
    "Creates a new async cached property where function will be called to "
    "produce\n"
...
skip_optional_pos:
  return_value = async_cached_property_init_impl(
      (PyAsyncCachedPropertyDescrObject*)self, func, name_or_descr);
...
PyDoc_STRVAR(
    async_cached_classproperty_new__doc__,
...
static PyObject* async_cached_classproperty_new_impl(
    PyTypeObject* type,
    PyObject* func);

static PyObject* async_cached_classproperty_new(
    PyTypeObject* type,
    PyObject* args,
    PyObject* kwargs) {
...
static int async_cached_classproperty_traverse(
...
async_cached_classproperty_get(PyObject* self, PyObject* obj, PyObject* cls) {
  PyAsyncCachedClassPropertyDescrObject* cp =
      (PyAsyncCachedClassPropertyDescrObject*)self;
  PyObject* res;
  ...
  if (res == NULL) {
    ...
    res =
        PyObject_CallFunctionObjArgs(_AsyncLazyValue_Type, cp->func, cls, NULL);
    if (res == NULL) {
      return NULL;
    }
    if (cp->value == NULL) {
...
static void async_cached_classproperty_dealloc(
    PyAsyncCachedClassPropertyDescrObject* cp) {
  PyObject_GC_UnTrack(cp);
  Py_XDECREF(cp->func);
  Py_XDECREF(cp->name);
...
static PyObject* async_cached_classproperty_get_name(
    PyAsyncCachedClassPropertyDescrObject* cp,
    void* closure) {
  PyObject* res = cp->name;
  Py_INCREF(res);
...
static PyGetSetDef async_cached_classproperty_getsetlist[] = {
    {"__doc__",
     (getter)async_cached_classproperty_get___doc__,
     NULL,
     NULL,
     NULL},
...
static PyMemberDef async_cached_classproperty_members[] = {
    {"func",
     T_OBJECT,
     offsetof(PyAsyncCachedClassPropertyDescrObject, func),
     READONLY},
    {0}};
