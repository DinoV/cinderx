 ...
 * DK_ENTRIES(dk)[index]. -1 when no entry found, -3 when compare raises error.
 */
...
struct _dictkeysobject {
  ...
  /* Function to lookup in the hash table (dk_indices):
     ...
     - lookdict_with_lazy_imports(): general-purpose with deferred values, may
     return DKIX_ERROR if (and only if) a comparison raises an exception. On
  ...
  /* "ChkDictKeyEntry dk_entries[dk_usable];" array follows:
...
#define Ci_CheckedDictKeys_Check(op) \
  PyObject_TypeCheck(op, &Ci_CheckedDictKeys_Type)
#define Ci_CheckedDictValues_Check(op) \
  PyObject_TypeCheck(op, &Ci_CheckedDictValues_Type)
#define Ci_CheckedDictItems_Check(op) \
  PyObject_TypeCheck(op, &Ci_CheckedDictItems_Type)
...
Py_ssize_t Ci_CheckedDict_KeysSize(ChkDictKeysObject* keys);
...
#define DICT_POP_METHODDEF                \
  {"pop",                                 \
   (PyCFunction)(void (*)(void))dict_pop, \
   METH_FASTCALL,                         \
   dict_pop__doc__},

static PyObject*
dict_pop_impl(CiChkDictObject* self, PyObject* key, PyObject* default_value);

static PyObject*
dict_pop(CiChkDictObject* self, PyObject* const* args, Py_ssize_t nargs) {
...
static PyObject* dict_popitem_impl(CiChkDictObject* self);

static PyObject* dict_popitem(
...
#define DICT___REVERSED___METHODDEF \
  {"__reversed__",                  \
   (PyCFunction)dict___reversed__,  \
   METH_NOARGS,                     \
   dict___reversed____doc__},

static PyObject* dict___reversed___impl(CiChkDictObject* self);

static PyObject* dict___reversed__(
    CiChkDictObject* self,
    PyObject* Py_UNUSED(ignored)) {
...
/* Changed only to rename some PyDict_* exposed elsewhere to CiCheckedDict_*,
 * and change some PyDict_Check to Ci_CheckedDict_Check. */

#define PERTURB_SHIFT 5
#define PyDict_MINSIZE 8

/* forward declarations */
static Py_ssize_t lookdict(
    CiChkDictObject* mp,
    PyObject* key,
    Py_hash_t hash,
    PyObject** value_addr,
    int resolve_lazy_imports);
static Py_ssize_t lookdict_unicode(
    CiChkDictObject* mp,
    PyObject* key,
    Py_hash_t hash,
    PyObject** value_addr,
    int resolve_lazy_imports);
static Py_ssize_t lookdict_unicode_nodummy(
    CiChkDictObject* mp,
    PyObject* key,
    Py_hash_t hash,
    PyObject** value_addr,
    int resolve_lazy_imports);
static Py_ssize_t lookdict_split(
    CiChkDictObject* mp,
    PyObject* key,
    Py_hash_t hash,
    PyObject** value_addr,
...
static int dictresize(CiChkDictObject* mp, Py_ssize_t newsize);
...
_Ci_dict_state _dict_state;

static _Ci_dict_state* get_dict_state(void) {
...
#else
#define DK_IXSIZE(dk) \
  (DK_SIZE(dk) <= 0xff ? 1 : DK_SIZE(dk) <= 0xffff ? 2 : sizeof(int32_t))
#endif
...
static inline void dictkeys_incref(ChkDictKeysObject* dk) {
#ifdef Py_REF_DEBUG
  _Py_RefTotal++;
#endif
  dk->dk_refcnt++;
...
static inline void dictkeys_decref(ChkDictKeysObject* dk) {
...
static inline Py_ssize_t dictkeys_get_index(
    const ChkDictKeysObject* keys,
    Py_ssize_t i) {
...
/* write to indices. */
static inline void
dictkeys_set_index(ChkDictKeysObject* keys, Py_ssize_t i, Py_ssize_t ix) {
  Py_ssize_t s = DK_SIZE(keys);
...
static inline Py_ssize_t calculate_keysize(Py_ssize_t minsize) {
...
#else
  Py_ssize_t size;
  for (size = PyDict_MINSIZE; size < minsize && size > 0; size <<= 1)
    ;
  return size;
#endif
}
 ...
 * resizing.
 */
static inline Py_ssize_t estimate_keysize(Py_ssize_t n) {
...
/* This immutable, empty ChkDictKeysObject is used for PyDict_Clear()
 * (which cannot fail and thus can do no allocation).
 */
static ChkDictKeysObject empty_keys_struct = {
    1, /* dk_refcnt */
    1, /* dk_size */
...
#define ASSERT_CONSISTENT(op) \
  assert(Ci_CheckedDict_CheckConsistency((PyObject*)(op), 1))
#else
#define ASSERT_CONSISTENT(op) \
  assert(Ci_CheckedDict_CheckConsistency((PyObject*)(op), 0))
#endif

int Ci_CheckedDict_CheckConsistency(PyObject* op, int check_content) {
#define CHECK(expr)                                        \
  do {                                                     \
    if (!(expr)) {                                         \
      _PyObject_ASSERT_FAILED_MSG(op, Py_STRINGIFY(expr)); \
    }                                                      \
  } while (0)
  ...
  if (check_content) {
    ChkDictKeyEntry* entries = DK_ENTRIES(keys);
    Py_ssize_t i;
    ...
    for (i = 0; i < usable; i++) {
      ChkDictKeyEntry* entry = &entries[i];
...
static ChkDictKeysObject* new_keys_object(Py_ssize_t size) {
  ChkDictKeysObject* dk;
  ...
  struct _Ci_dict_state* state = get_dict_state();
#ifdef Py_DEBUG
  // new_keys_object() must not be called after _PyDict_Fini()
  assert(state == NULL || state->keys_numfree != -1);
#endif
  if (size == PyDict_MINSIZE && state != NULL && state->keys_numfree > 0) {
    dk = state->keys_free_list[--state->keys_numfree];
  } else {
    dk = PyObject_Malloc(
        sizeof(ChkDictKeysObject) + es * size +
  ...
  memset(DK_ENTRIES(dk), 0, sizeof(ChkDictKeyEntry) * usable);
  return dk;
}
...
static ChkDictKeysObject* clone_combined_dict_keys(CiChkDictObject* orig) {
  ...
  ChkDictKeysObject* keys = PyObject_Malloc(keys_size);
  if (keys == NULL) {
    PyErr_NoMemory();
    return NULL;
  }
  ...
  /* After copying key/value pairs, we need to incref all
     ...
     new dict object. */
  ChkDictKeyEntry* ep0 = DK_ENTRIES(keys);
  Py_ssize_t n = keys->dk_nentries;
  for (Py_ssize_t i = 0; i < n; i++) {
    ChkDictKeyEntry* entry = &ep0[i];
...
static Py_ssize_t
lookdict_index(ChkDictKeysObject* k, Py_hash_t hash, Py_ssize_t index) {
...
*/
static Py_ssize_t _Py_HOT_FUNCTION lookdict(
    CiChkDictObject* mp,
    PyObject* key,
    Py_hash_t hash,
    PyObject** value_addr,
...
static Py_ssize_t _Py_HOT_FUNCTION lookdict_unicode(
    CiChkDictObject* mp,
    PyObject* key,
    Py_hash_t hash,
    PyObject** value_addr,
  ...
  ChkDictKeyEntry* ep0 = DK_ENTRIES(mp->ma_keys);
 ...
 * will be present. */
static Py_ssize_t _Py_HOT_FUNCTION lookdict_unicode_nodummy(
    CiChkDictObject* mp,
    PyObject* key,
    Py_hash_t hash,
    PyObject** value_addr,
 ...
 */
static Py_ssize_t _Py_HOT_FUNCTION lookdict_split(
    CiChkDictObject* mp,
    PyObject* key,
    Py_hash_t hash,
    PyObject** value_addr,
...
static Py_ssize_t find_empty_slot(ChkDictKeysObject* keys, Py_hash_t hash) {
  assert(keys != NULL);
...
static int insertion_resize(CiChkDictObject* mp) {
...
static int insert_to_emptydict(
    CiChkDictObject* mp,
    PyObject* key,
    Py_hash_t hash,
    PyObject* value) {
  ...
  ChkDictKeyEntry* ep = DK_ENTRIES(mp->ma_keys);
...
*/
static void
build_indices(ChkDictKeysObject* keys, ChkDictKeyEntry* ep, Py_ssize_t n) {
...
static int dictresize(CiChkDictObject* mp, Py_ssize_t newsize) {
  ...
  ChkDictKeysObject* oldkeys;
  PyObject** oldvalues;
  ChkDictKeyEntry *oldentries, *newentries;
  ...
  if (oldvalues != NULL) {
    ...
    for (Py_ssize_t i = 0; i < numentries; i++) {
      ...
      ChkDictKeyEntry* ep = &oldentries[i];
    ...
    if (oldkeys->dk_nentries == numentries) {
      memcpy(newentries, oldentries, numentries * sizeof(ChkDictKeyEntry));
    } else {
...
void Ci_CheckedDict_Clear(PyObject* op) {
  CiChkDictObject* mp;
  ChkDictKeysObject* oldkeys;
  PyObject** oldvalues;
  Py_ssize_t i, n;
...
static PyObject* Ci_CheckedDict_Pop_KnownHash(
    PyObject* dict,
    PyObject* key,
    Py_hash_t hash,
    PyObject* deflt) {
...
Ci_CheckedDict_Pop(PyObject* dict, PyObject* key, PyObject* deflt) {
  ...
  if (((CiChkDictObject*)dict)->ma_used == 0) {
    if (deflt) {
      Py_INCREF(deflt);
      return deflt;
    }
...
static void dict_dealloc(CiChkDictObject* mp) {
  ...
  Py_TRASHCAN_BEGIN(mp, dict_dealloc) if (values != NULL) {
...
static PyObject* dict_repr(CiChkDictObject* mp) {
  Py_ssize_t i;
...
static Py_ssize_t dict_length(CiChkDictObject* mp) {
  return mp->ma_used;
}

static PyObject* dict_subscript(CiChkDictObject* mp, PyObject* key) {
  Py_ssize_t ix;
  Py_hash_t hash;
  PyObject* value;
...
static int dict_merge(PyObject* a, PyObject* b, int override) {
  ...
  if (Ci_CheckedDict_Check(b) &&
    ...
    if (mp->ma_used == 0) {
       ...
       */
      override = 1;
      ChkDictKeysObject* okeys = other->ma_keys;

      // If other is clean, combined, and just allocated, just clone it.
      if (other->ma_values == NULL && other->ma_used == okeys->dk_nentries &&
...
static int dict_equal_pydict(CiChkDictObject* a, PyObject* b) {
  ...
  for (i = 0; i < a->ma_keys->dk_nentries; i++) {
    ...
    if (aval != NULL) {
      ...
      Py_INCREF(key);
      /* reuse the known hash value */
      bval = _PyDict_GetItem_KnownHash(b, key, ep->me_hash);
      if (bval == NULL) {
        Py_DECREF(key);
        Py_DECREF(aval);
        if (PyErr_Occurred()) {
...
static PyObject* dict___contains__(CiChkDictObject* self, PyObject* key) {
  register CiChkDictObject* mp = self;
  Py_hash_t hash;
  Py_ssize_t ix;
  PyObject* value;
...
static PyObject*
dict_get_impl(CiChkDictObject* self, PyObject* key, PyObject* default_value) {
  PyObject* val = NULL;
  Py_hash_t hash;
  Py_ssize_t ix;
...
Ci_CheckedDict_SetDefault(PyObject* d, PyObject* key, PyObject* defaultobj) {
  CiChkDictObject* mp = (CiChkDictObject*)d;
  PyObject* value;
  Py_hash_t hash;
...
Py_ssize_t Ci_CheckedDict_KeysSize(ChkDictKeysObject* keys) {
  return (
      sizeof(ChkDictKeysObject) + DK_IXSIZE(keys) * DK_SIZE(keys) +
      USABLE_FRACTION(DK_SIZE(keys)) * sizeof(ChkDictKeyEntry));
}

Py_ssize_t _CiDict_SizeOf(CiChkDictObject* mp) {
...
static PyObject* dict_sizeof(
    CiChkDictObject* mp,
    PyObject* Py_UNUSED(ignored)) {
  return PyLong_FromSsize_t(_CiDict_SizeOf(mp));
}
...
static int CiCheckedDict_Contains(PyObject* op, PyObject* key) {
  Py_hash_t hash;
  Py_ssize_t ix;
  CiChkDictObject* mp = (CiChkDictObject*)op;
  PyObject* value;
...
static PyObject*
chknew_dict(PyTypeObject* type, ChkDictKeysObject* keys, PyObject** values) {
  _Ci_dict_state* state = get_dict_state();
  CiChkDictObject* mp;
  assert(keys != NULL);
  if (state->numfree) {
    ...
    assert(mp != NULL);
    ((PyObject*)mp)->ob_type = type;
    _Py_NewReference((PyObject*)mp);
    /* Generic types are a heap allocated, so we need to bump the
     * ref count here. */
    Py_INCREF(type);
  } else {
    mp = PyObject_GC_New(CiChkDictObject, type);
    if (mp == NULL) {
      dictkeys_decref(keys);
      if (values != empty_values) {
        free_values(values);
...
static PyObject* chkdict_alloc(PyTypeObject* type, Py_ssize_t nitems) {
...
static inline int chkdict_checkkey(CiChkDictObject* d, PyObject* key) {
...
static inline int chkdict_checkval(CiChkDictObject* d, PyObject* value) {
...
static int
chkdict_ass_sub_unchecked(CiChkDictObject* mp, PyObject* key, PyObject* value) {
  /* we can't use PyDict_SetItem/DelItem directly as they check
   * that we have a dictionary type */
...
static PyObject* chkdict_subscript(CiChkDictObject* mp, PyObject* key) {
  if (chkdict_checkkey(mp, key)) {
    return NULL;
  }
...
static int chkdict_merge_iterable(PyObject* a, PyObject* b) {
  /* Do it the generic, slower way */
  PyObject* keys = PyMapping_Keys(b);
  PyObject* iter;
  PyObject *key, *value;
...
static int chkdict_merge(PyObject* a, PyObject* b) {
  CiChkDictObject *mp, *other;
  Py_ssize_t i, n;
  ChkDictKeyEntry *entry, *ep0;
  ...
  if (Py_TYPE(b)->tp_iter == (getiterfunc)dict_iter) {
    ...
    if (chkdict_merge_iterable(a, b) < 0) {
      return -1;
    }
  }
  ASSERT_CONSISTENT(a);
...
int chkdict_mergefromseq2(PyObject* d, PyObject* seq2) {
  ...
  for (i = 0;; ++i) {
    ...
    Py_INCREF(value);
    int status;
    if (chkdict_checkkey((CiChkDictObject*)d, key) ||
        chkdict_checkval((CiChkDictObject*)d, value)) {
      status = -1;
    } else {
      status = insertdict((CiChkDictObject*)d, key, PyObject_Hash(key), value);
    }
    if (status < 0) {
      Py_DECREF(key);
      Py_DECREF(value);
...
static int chkdict_update_common(
    PyObject* self,
    PyObject* args,
    PyObject* kwds,
    const char* methname) {
...
static int chkdict_init(PyObject* self, PyObject* args, PyObject* kwds) {
...
const Ci_Py_SigElement* const setitem_sig[] = {
    &Ci_Py_Sig_T0,
    &Ci_Py_Sig_T1,
    NULL};
Ci_PyTypedMethodDef chkdict_setitem_def = {
    chkdict_ass_sub_unchecked,
    setitem_sig,
    Ci_Py_SIG_ERROR};
...
static PyObject* chkdict_richcompare(PyObject* v, PyObject* w, int op) {
  int cmp;
  PyObject* res;
...
typedef struct {
  PyObject_HEAD
  CiChkDictObject* di_dict; /* Set to NULL when iterator is exhausted */
  Py_ssize_t di_used;
  Py_ssize_t di_pos;
...
static PyObject* dictiter_new(CiChkDictObject* dict, PyTypeObject* itertype) {
  dictiterobject* di;
...
static PyMethodDef dictiter_methods[] = {
    {"__length_hint__",
     (PyCFunction)(void (*)(void))dictiter_len,
     METH_NOARGS,
     length_hint_doc},
    {"__reduce__",
     (PyCFunction)(void (*)(void))dictiter_reduce,
     METH_NOARGS,
     reduce_doc},
    {NULL, NULL} /* sentinel */
};
...
static PyObject* dictiter_iternextkey(dictiterobject* di) {
  ...
  if (d->ma_values) {
    ...
    ChkDictKeyEntry* entry_ptr = &DK_ENTRIES(k)[i];
  ...
  if (di->len == 0) {
    PyErr_SetString(
        PyExc_RuntimeError, "dictionary keys changed during iteration");
    goto fail;
  }
  di->di_pos = i + 1;
  di->len--;
...
static PyObject* dictreviter_iternext(dictiterobject* di) {
  PyObject** value_ptr;
  ChkDictKeyEntry* entry_ptr;
  CiChkDictObject* d = di->di_dict;
  ...
  ChkDictKeysObject* dk = d->ma_keys;
  PyObject *key, *value, *result;
...
static void dictview_dealloc(_CiDictViewObject* dv) {
  /* bpo-31095: UnTrack is needed before calling any callbacks */
...
static int
dictview_traverse(_CiDictViewObject* dv, visitproc visit, void* arg) {
  Py_VISIT(dv->dv_dict);
  return 0;
}
...
static PyObject* Ci_CheckedDictView_New(PyObject* dict, PyTypeObject* type) {
  CiChkDictObject* d;
  _CiDictViewObject* dv;
  if (dict == NULL) {
    PyErr_BadInternalCall();
    return NULL;
  }
...
static PyObject* dictview_mapping(PyObject* view, void* Py_UNUSED(ignored)) {
  assert(view != NULL);
  assert(
      Ci_CheckedDictKeys_Check(view) || Ci_CheckedDictValues_Check(view) ||
      Ci_CheckedDictItems_Check(view));
  PyObject* mapping = (PyObject*)((_CiDictViewObject*)view)->dv_dict;
...
static PyObject* dictview_richcompare(PyObject* self, PyObject* other, int op) {
...
static PyObject* dictview_repr(_CiDictViewObject* dv) {
  PyObject* seq;
...
static PyObject* dictkeys_iter(_CiDictViewObject* dv) {
  if (dv->dv_dict == NULL) {
    Py_RETURN_NONE;
  }
...
static PyObject* dictviews_to_set(PyObject* self) {
  return PySet_New(self);
}

static PyObject* dictviews_sub(PyObject* self, PyObject* other) {
  ...
  PyObject* tmp =
      _PyObject_CallMethodOneArg(result, s_difference_update, other);
  if (tmp == NULL) {
    Py_DECREF(result);
    return NULL;
  }
...
static PyObject* Ci_CheckedDictView_Intersect(PyObject* self, PyObject* other) {
  PyObject* result;
  PyObject* it;
  PyObject* key;
  Py_ssize_t len_self;
  int rv;
  int (*dict_contains)(_CiDictViewObject*, PyObject*);
  ...
  len_self = dictview_len((_CiDictViewObject*)self);
  ...
  if (Ci_CheckedDictViewSet_Check(other)) {
    Py_ssize_t len_other = dictview_len((_CiDictViewObject*)other);
  ...
  while ((key = PyIter_Next(it)) != NULL) {
    rv = dict_contains((_CiDictViewObject*)self, key);
    if (rv < 0) {
      goto error;
    }
    if (rv) {
...
static PyObject* dictviews_or(PyObject* self, PyObject* other) {
  PyObject* result = dictviews_to_set(self);
  if (result == NULL) {
    return NULL;
  }
...
static PyObject* dictitems_xor(PyObject* self, PyObject* other) {
  assert(Ci_CheckedDictItems_Check(self));
  assert(Ci_CheckedDictItems_Check(other));
  PyObject* d1 = (PyObject*)((_CiDictViewObject*)self)->dv_dict;
  PyObject* d2 = (PyObject*)((_CiDictViewObject*)other)->dv_dict;
...
static PyObject* dictviews_xor(PyObject* self, PyObject* other) {
  ...
  PyObject* tmp =
      _PyObject_CallMethodOneArg(result, s_symmetric_difference_update, other);
  if (tmp == NULL) {
    Py_DECREF(result);
    return NULL;
  }
...
static PyObject* dictviews_isdisjoint(PyObject* self, PyObject* other) {
  PyObject* it;
  PyObject* item = NULL;
...
static PyObject* dictkeys_reversed(
    _CiDictViewObject* dv,
    PyObject* Py_UNUSED(ignored));
...
static PyMethodDef dictkeys_methods[] = {
    {"isdisjoint", (PyCFunction)dictviews_isdisjoint, METH_O, isdisjoint_doc},
    {"__reversed__",
     (PyCFunction)(void (*)(void))dictkeys_reversed,
     METH_NOARGS,
     reversed_keys_doc},
    {NULL, NULL} /* sentinel */
};
...
static PyObject* dictitems_iter(_CiDictViewObject* dv) {
  if (dv->dv_dict == NULL) {
    Py_RETURN_NONE;
  }
...
static PyObject* dictitems_reversed(
    _CiDictViewObject* dv,
    PyObject* Py_UNUSED(ignored));
...
static PyMethodDef dictitems_methods[] = {
    {"isdisjoint", (PyCFunction)dictviews_isdisjoint, METH_O, isdisjoint_doc},
    {"__reversed__",
     (PyCFunction)dictitems_reversed,
     METH_NOARGS,
     reversed_items_doc},
    {NULL, NULL} /* sentinel */
};
...
static PyObject* dictitems_new(PyObject* dict, PyObject* Py_UNUSED(ignored)) {
...
static PyObject* dictvalues_iter(_CiDictViewObject* dv) {
  if (dv->dv_dict == NULL) {
    Py_RETURN_NONE;
  }
...
static PyObject* dictvalues_reversed(
    _CiDictViewObject* dv,
    PyObject* Py_UNUSED(ignored));
...
static PyMethodDef dictvalues_methods[] = {
    {"__reversed__",
     (PyCFunction)dictvalues_reversed,
     METH_NOARGS,
     reversed_values_doc},
    {NULL, NULL} /* sentinel */
};
...
static PyObject* dictvalues_new(PyObject* dict, PyObject* Py_UNUSED(ignored)) {
