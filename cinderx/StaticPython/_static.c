...
static int _static_exec(PyObject* m) {
  ...
  if ((Ci_CheckedListRevIter_Type = (PyTypeObject*)PyType_FromSpec(
      ...
      PyType_Ready(&_PyClassLoader_VTableInitThunk_Type) < 0 ||
      PyType_Ready(&_PyTypedArgsInfo_Type) < 0 ||
      PyType_Ready(&_PyType_VTableType) < 0 ||
      PyType_Ready(&_PyType_TypeCheckThunk) < 0 ||
      PyType_Ready(&_PyClassLoader_LazyFuncJitThunk_Type) < 0 ||
      PyType_Ready(&_PyClassLoader_StaticMethodThunk_Type) < 0 ||
      PyType_Ready(&_PyClassLoader_ClassMethodThunk_Type) < 0) {
    return -1;
  }
